...
/// - свёртка по `A → a` — для каждого терминала из FOLLOW(A) на завершённом пункте
/// - приём — на завершённом пункте продукции аксиомы при конце входа
///
...
public struct ParseTable {
    ...
    public init(_ grammar: ExpandedGrammar, acceptsEmpty: Bool) throws {
        ...
        /// Завершённые пункты: свёртки по FOLLOW и приём для аксиомы
        for (state, items) in automaton.states.enumerated() {
            for item in items {
                let production = numbered.productions[item.production]
                
                /// Незавершённый пункт свёртки не даёт
                guard item.position == production.rhs.count else { continue }
                
                /// Завершённый пункт продукции аксиомы (номер 0) — приём по концу входа
                if production.id == 0 {
                    append(
                        .accept,
                        to: &action,
                ...
                /// Свёртка по этой продукции для каждого терминала из FOLLOW левой части
